...
describe('AcadeGrade landing presentation', () => {
  ...
  it('explains the calculation boundary without implying institutional certification', () => {
    ...
    expect(stages[0]).toHaveClass('academic-proof-card');
    expect(stages[1]).toHaveClass('academic-proof-card');
    expect(stages[2]).toHaveClass('academic-proof-card');
    expect(stages[3]).toHaveClass('academic-proof-card');
    expect(stages[0]).toHaveStyle({ '--deck-card-y': '0.00px', '--deck-card-opacity': '1.000' });
    expect(stages[1]).toHaveStyle({ '--deck-card-y': '520.00px', '--deck-card-opacity': '0.000' });
    expect(stages[2]).toHaveStyle({ '--deck-card-y': '520.00px', '--deck-card-opacity': '0.000' });
    expect(stages[3]).toHaveStyle({ '--deck-card-y': '520.00px', '--deck-card-opacity': '0.000' });
